...
- **ทุกครั้งที่แก้ `app.js` หรือ `styles.css` ต้องเลื่อนเลข `?v=` ใน `index.html` ด้วย** ทั้งสองไฟล์ใช้ป้ายรุ่นชุดเดียวกัน (`?v=20260921d`) เพราะเปลี่ยนคู่กันเสมอ ถ้าไม่เลื่อน เบราว์เซอร์ที่เคยเปิดเว็บแล้วจะยังใช้ไฟล์เก่าจาก cache ต่อไป แล้วดูเหมือน deploy ไม่ติด — GitHub Pages ไม่มี build step จึงไม่มี content hash ให้อัตโนมัติ ต้องเลื่อนเอง (ใช้วันที่ deploy เป็นเลขรุ่นได้เลย)
...
## workflow แจ้งซ่อมเต็มรูปแบบ (ตรงสเปกระบบเดิม)

`20260921090000_repair_workflow_spec_alignment.sql` เก็บส่วนที่เหลือทั้งหมดให้ตรงสเปกของระบบเดิม
ต่อจาก `20260921080000` ที่แก้เฉพาะสายอนุมัติ

| เรื่อง | เดิม | ตอนนี้ |
| --- | --- | --- |
| ช่างผู้รับผิดชอบ | `assignee_id` คนเดียว | ตาราง `request_technicians` เลือกได้หลายคน แจ้งเตือนทุกคน (`assignee_id` ยังอยู่ = ช่างคนแรกของชุด เพื่อไม่ให้ RLS/หน้าจอ/สำเนาชีตเดิมพัง) |
| เปลี่ยนช่าง | ทำได้เฉพาะตอน `pending_assign` | แก้รายชื่อได้ทุกสถานะ ยกเว้นใบที่ `rejected` หรือยังไม่ผ่านอนุมัติ |
| ฟิลด์ของ ผจก.ซ่อมบำรุง | "การดำเนินงาน"/"ความคิดเห็นช่างผู้ตรวจสอบ" อยู่ในฟอร์มจบงานของช่าง | ย้ายมาอยู่ขั้นมอบหมาย พร้อมช่องใหม่ `received_by_name` (ชื่อผู้จัดการที่รับใบ) |
| ฟอร์มจบงานของช่าง | 4 ช่อง | เหลือ "วิเคราะห์สาเหตุ + รายการอะไหล่" ตามที่ยืนยัน |
| หมุดความคืบหน้า | ไม่มี | ตาราง `request_progress_steps` + RPC `app_record_progress_step` |
| เหตุผลตอนปฏิเสธ | ไม่บังคับ | "ไม่อนุมัติ" และ "ขอข้อมูลเพิ่มเติม" ต้องระบุ ≥3 ตัวอักษร (`COMMENT_REQUIRED`) |
| วันเริ่ม/วันคาดว่าเสร็จ | ไม่บังคับ | บังคับตอนมอบหมายครั้งแรก และวันเสร็จต้องไม่ก่อนวันเริ่ม |
| แจ้งเตือน | ซ่อมเสร็จแจ้งแค่ผู้แจ้ง · ตรวจรับผ่านไม่แจ้งใคร | ซ่อมเสร็จแจ้งผู้แจ้ง + ผจก.ซ่อมบำรุง · ตรวจรับ (ทั้งผ่านและไม่ผ่าน) แจ้งช่างทุกคน + ผจก.ซ่อมบำรุง · อนุมัติครบแจ้ง ผจก.ซ่อมบำรุงให้มามอบหมาย |
| ใบด่วน | เก็บ `is_urgent` ไว้เฉยๆ | หัวแจ้งเตือนขึ้น `🚨 [ด่วน]` นำหน้า (`private.notify_title`) |
| ชื่อสถานะ | `pending_approval`/`more_info` ตัวเดียว | หน้ารายละเอียดเติมชื่อขั้นต่อท้าย เช่น "รออนุมัติ (ผจก.โรงงาน)" ให้อ่านได้เหมือนระบบเดิมที่แยก `PENDING_FM`/`PENDING_GM` |
...
| มอบหมาย/เปลี่ยนช่าง | ผจก.แผนกซ่อมบำรุง (+ `requests.view_all`) |
...
หมุดไม่บล็อกการกดเริ่มซ่อมหรือจบงาน เป็นการรายงานความคืบหน้าอย่างเดียว ทุกครั้งที่กดจะแจ้งผู้แจ้ง
และ ผจก.ซ่อมบำรุงให้ทราบ
