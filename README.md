...
- **ทุกครั้งที่แก้ `app.js` หรือ `styles.css` ต้องเลื่อนเลข `?v=` ใน `index.html` ด้วย** ทั้งสองไฟล์ใช้ป้ายรุ่นชุดเดียวกัน (`?v=20260921b`) เพราะเปลี่ยนคู่กันเสมอ ถ้าไม่เลื่อน เบราว์เซอร์ที่เคยเปิดเว็บแล้วจะยังใช้ไฟล์เก่าจาก cache ต่อไป แล้วดูเหมือน deploy ไม่ติด — GitHub Pages ไม่มี build step จึงไม่มี content hash ให้อัตโนมัติ ต้องเลื่อนเอง (ใช้วันที่ deploy เป็นเลขรุ่นได้เลย)
