...
- ลำดับอนุมัติคำร้องทั่วไป: หัวหน้าแผนก → ผู้อนุมัติของหน่วยงานรับผิดชอบ
- ลำดับอนุมัติใบแจ้งซ่อม (`uses_repair_workflow`): ผู้จัดการโรงงาน → ผู้จัดการทั่วไป แล้วเข้าสู่ `pending_assign` (ดู "ลำดับอนุมัติใบแจ้งซ่อม" ด้านล่าง)
...
## ลำดับอนุมัติใบแจ้งซ่อม

ใบแจ้งซ่อม (`request_types.uses_repair_workflow = true` ปัจจุบันคือโมดูล `MT_REPAIR`) **ไม่ได้** ใช้
ผู้อนุมัติแบบปกติของคำร้องทั่วไปแล้ว ขั้น "หัวหน้าแผนก" และ "ผู้อนุมัติหน่วยงานรับผิดชอบ" ถูกยกเลิก
ออกจากใบแจ้งซ่อมตั้งแต่ไมเกรชัน `20260919030000_repair_approval_factory_general_manager.sql`
และแทนด้วยผู้อนุมัติสองระดับตามระบบแจ้งซ่อม MT เดิม

```text
สร้างใบแจ้งซ่อม (pending_approval, current_step = 1)
   ↓ ขั้นที่ 1 · ผู้จัดการโรงงาน   (roles.code = 'factory_manager')
   ↓ ขั้นที่ 2 · ผู้จัดการทั่วไป    (roles.code = 'general_manager')
pending_assign → assigned → in_progress → pending_verify → completed
```

- ทุกขั้นอนุมัติผูกกับ **บทบาท** ไม่จำกัดแผนก ไม่ได้ผูกกับผู้บังคับบัญชาของผู้แจ้ง
- ปฏิเสธที่ขั้นใดก็ได้ → `rejected` · ขอข้อมูลเพิ่มที่ขั้นใดก็ได้ → `more_info`
- ป้ายสถานะบนหน้าจอใบแจ้งซ่อมบอกด้วยว่าค้างอยู่ที่ใคร เช่น `รออนุมัติ (ผจก.โรงงาน)` และ
  `ต้องการข้อมูลเพิ่มเติม (ผจก.ทั่วไป)`
- ผู้ถือบทบาทผู้จัดการทั้งสองได้สิทธิ์อนุมัติทุกโมดูลที่เปิดใช้งานโดยอัตโนมัติ (trigger
  `employees_manager_module_permissions`) เพื่อไม่ให้ติดกับดัก `approval_module_permissions`
  ว่างแล้วมองไม่เห็นใบที่รอตัวเองอยู่ — Admin ถอดสิทธิ์รายโมดูลทีหลังได้ตามปกติ
- ถ้ายังไม่มีพนักงานคนไหนถือบทบาทผู้จัดการโรงงาน/ผู้จัดการทั่วไป ขั้นนั้นจะไม่มีผู้ถือบทบาทมากด
  อนุมัติ แต่ใบไม่ค้างถาวรเพราะบัญชีที่ถือสิทธิ์ `requests.view_all` (Admin) อนุมัติแทนได้ทุกขั้น
- ใบที่สร้างก่อนไมเกรชันนี้ยังคงขั้นอนุมัติแบบเดิมของตัวเองไว้ เพื่อไม่ให้ประวัติการอนุมัติจริงถูก
  เขียนทับ ลำดับใหม่มีผลกับใบที่สร้างหลังจากนั้น
